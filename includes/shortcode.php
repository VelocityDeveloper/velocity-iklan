	...
	$html .= '<article class="'.esc_attr(implode(' ',$post_classes)).'" id="post-'.$post_id.'">';
		$html .= '<div class="entry-content">';
			...
			$html .= '<div class="col-content">';
				$html .= '<div class="fw-bold mb-2"><a href="'.get_the_permalink($post_id).'" rel="bookmark">'.get_the_title($post_id).'</a></div>';
				$html .= '<div class="mb-1"><strong>Harga:</strong> '.do_shortcode('[velocity-iklan-harga post_id="'.$post_id.'"]').'</div>';
				$html .= do_shortcode('[velocity-iklan-taxonomy taxonomy="lokasi" post_id="'.$post_id.'"]');
			$html .= '</div>';
		$html .= '</div>';
	$html .= '</article>';
    return $html;
